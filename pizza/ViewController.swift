...
class ViewController: UIViewController, sizePizzaDelegate, doughPizzaDelegate, cheesePizzaDelegate, ingredientsPizzaDelegate, orderPizzaDelegate {
    ...
    @IBAction func btnConfirmOrder(sender: AnyObject) {
        if validateSelectionSteps(){
            let viewOrdenController = self.storyboard?.instantiateViewControllerWithIdentifier("ConfirmOrder") as? ViewOrdenController
            viewOrdenController!.delegate = self
            viewOrdenController!.orderPizza = customPizza
            self.navigationController?.pushViewController(viewOrdenController!, animated: true)
